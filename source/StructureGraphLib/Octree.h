...
public:
    ...
    Eigen::Vector3d closestIntersectionPoint( const Ray & ray, int * faceIndex, bool isRobsut = false )
    {
        ...
        {
            intersectionTestAccelerated(opengp::Surface_mesh::Face(i), ray, res);
        ...
        if( isRobsut )
        {
            ...
            if(!best_res.hit){
                foreach( int i, intersectRay( ray, 0.01, false ) ){
                    rayTriangleIntersectionTest(opengp::Surface_mesh::Face(i), ray, res, false);
                    if(res.hit){
                        if (res.distance < minDistance){
                            minDistance = res.distance;
                            isetpoint = ray.origin + (ray.direction * res.distance);
...
private:
    ...
    void initBuild( std::vector<Surface_mesh::Face>& tris, int triPerNode )
    {
        ...
        for(auto fit : mesh->faces()){
            ...
            triangles[opengp::Surface_mesh::Face(fit).idx()] = pnts;
        }

        // Pre-compute per triangle quantities for fast ray-intersection
#if USE_TRI_ACCEL
        auto tri_accel = mesh->face_property<TriAcceld>("f:TriAccel");
        for(auto fit : mesh->faces()){
            int i = opengp::Surface_mesh::Face(fit).idx();
...
public:
    ...
    {
        std::vector<Eigen::Vector3d> pnts;
        for(auto vit : mesh->vertices(f)) pnts.push_back(points[vit]);
        return pnts;
    }
