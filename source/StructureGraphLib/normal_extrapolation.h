...
private:
    ...
    struct Plane
    {
        Plane() { center = Vector3(0,0,0); normal = Vector3(0,0,0); }
    ...
    struct MSTNode
    {
        ...
        MSTNode					*parent;
        opengp::Surface_mesh::Vertex		vertex;
...
public:
                                   ...
                                   unsigned int k, const int root_index=-1, NormalOrientation orientation = Guess)
    {
        ...
        for(auto v : m.vertices()) normals.push_back( out_normals[v] );
    }
    ...
    static void ExtrapolateNormals(SurfaceMeshModel * mesh, unsigned int k, const int root_index=-1, NormalOrientation orientation = Guess)
    {
        ...
        Vector3VertexProperty normals = mesh->vertex_property<Vector3>(VNORMAL, Vector3(0,0,0));
        ...
        auto max_distance = dataset_bb.diagonal().norm();
        ...
        for(auto v : mesh->vertices()) kdtree_for_planes.addPoint(points[v]);
        ...
        for(auto v : mesh->vertices())
        {
            ...
            for (unsigned int n = 0; n < k; n++)
                plane->center += points[opengp::Surface_mesh::Vertex(matches[n].first)];
            ...
            for (unsigned int n = 0; n < k; n++)
            {
                Vector3 diff = points[ opengp::Surface_mesh::Vertex(matches[n].first) ] - plane->center;
                for (int i=0; i<3; i++)
                    for (int j=0; j<3; j++)
        ...
        for (PlaneIterator iPlane = tangent_planes.begin(); iPlane != ePlane; iPlane++)
        {
            ...
            for (unsigned int n = 0; n < k; n++)
            {
                ...
                if (iPlane->index < idx)
                    riemannian_graph[iPlane->index].push_back(RiemannianEdge(&tangent_planes[idx], 1.0 - abs(iPlane->normal.dot(tangent_planes[idx].normal))));
            }
        }
        ...
        { // just to limit the scope of the variable border
            ...
            while ((queueSize=int(border.size())) > 0)
            {
                ...
                current_node->vertex	= opengp::Surface_mesh::Vertex(current_node_index);	// and associate it to the MST node
        ...
        // and finally visit the MST tree in order to propagate normal flips
        {
            ...
            while ((queueSize = int(border.size())) > 0)
            {
                ...
                {
                    if (normals[current_node->vertex].dot(normals[current_node->sons[s]->vertex]) < 0.0)
        ...
        if(orientation == Guess)
        {
            ...
            Vector3 closestNormal = normals[opengp::Surface_mesh::Vertex(match[0].first)];

            if(direction.dot(closestNormal) < 0){
                for(auto v : mesh->vertices())
                    normals[v] *= -1.0;
            }
        }
    ...
    static void SmoothNormalsUsingNeighbors(SurfaceMeshModel * mesh, const unsigned int k, bool usedistance)
    {
        ...
        for(auto v : mesh->vertices()) tree_for_neighbors.addPoint(points[v]);
        tree_for_neighbors.build();

        std::vector< Vector3 > new_normals(vertex_count);
        ...
        for(auto v : mesh->vertices())
        {
            ...
            Vector3 normal_accum(0.0, 0.0, 0.0);

            auto dist_max = matches.back().second;

            for (unsigned int n = 0; n < k; n++){
                if(usedistance)
                    normal_accum += (normals[opengp::Surface_mesh::Vertex(matches[n].first)] * matches[n].second / dist_max);
                else
                    normal_accum += normals[opengp::Surface_mesh::Vertex(matches[n].first)];
            }
